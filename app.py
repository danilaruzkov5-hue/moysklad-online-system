...
with t4:
    st.subheader("📊 Ежедневный расчет хранения (16 кор. = 1 паллет = 50₽)")
    
    # Загружаем историю из Neon
    try:
        history = pd.read_sql("SELECT * FROM daily_storage_logs ORDER BY log_date DESC", engine)
        
        if not history.empty:
            # Названия колонок как в ТЗ у Дмитрия
            history.columns = [
                "Дата", "Кор. ИП", "Паллет ИП", "₽ ИП", 
                "Кор. ООО", "Паллет ООО", "₽ ООО", "ИТОГО ₽"
            ]
            
            # Настройка отображения: красим таблицу
            st.dataframe(
            ...
            st.info("История будет создана автоматически сегодня в 23:00.")
