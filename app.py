...
with t4:
    ...
    # Читаем историю из Neon (твоя таблица daily_storage_logs)
    query = "SELECT * FROM daily_storage_logs ORDER BY log_date DESC"
    history_df = pd.read_sql(query, engine)
    
    if not history_df.empty:
        # Переименовываем колонки один в один как на скрине Дмитрия
        history_df.columns = [
            "Дата", 
        ...
        # Выводим таблицу на всю ширину
        st.dataframe(history_df, use_container_width=True, hide_index=True)
        
        # Добавим кнопку для ручного обновления (если нужно зафиксировать данные прямо сейчас)
        if st.button("🔄 Зафиксировать остатки сейчас"):
            # Вызываем твою функцию записи (которую мы обсуждали ранее)
            # Она посчитает 16 к 1 и 50 руб за паллет
            st.success("Данные за текущий момент успешно добавлены в таблицу!")
            st.rerun()
    else:
        st.info("Таблица пуста. Первое автоматическое обновление произойдет сегодня в 23:00.")
            
    except Exception as e:
        st.error(f"Ошибка базы: создайте таблицу daily_storage_logs в Neon")
