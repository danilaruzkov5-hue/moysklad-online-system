...
def render_table(storage_type, key):
    ...
    if not display_df.empty:
        ...
        # 4. Логика сохранения выбора
        selection = sel.get("selection", {})
        current_rows = selection.get("rows", [])

        # Безопасно получаем UUID того, что выбрано прямо сейчас на экране
        max_idx = len(display_df) - 1
        currently_selected_on_screen = [
            display_df.iloc[r]['uuid'] for r in current_rows if r <= max_idx
        ]

        # Набор UUID, которые сейчас видны пользователю
        visible_uuids = set(display_df['uuid'].tolist())

        # ОБНОВЛЯЕМ ПАМЯТЬ:
        # Добавляем новые галочки в сессию
        ...
        # 5. Считаем общий итог для кнопок
